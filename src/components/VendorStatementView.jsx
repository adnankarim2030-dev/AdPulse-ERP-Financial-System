            ...
            <div>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", borderBottom: "1.5px solid #0F172A", paddingBottom: 10, marginBottom: 12 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: 10, color: "#475569", fontWeight: 600 }}>Financial Management &amp; Accounts Payable Sub-Ledger</div>
                  </div>
                </div>
                <div style={{ textAlign: "right" }}>
                  <div style={{ fontSize: 13.5, fontWeight: 800, color: "#D97706", textTransform: "uppercase", letterSpacing: "0.5px" }}>VENDOR STATEMENT</div>
                  <div className="mono" style={{ fontSize: 10.5, fontWeight: 700, color: "#0F172A" }}>STMT-{(vendor.vendorCode || vendor.id || "").toUpperCase()}</div>
                </div>
              </div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 8, marginBottom: 8 }}>
                <div style={{ background: "#FFFFFF", padding: "6px 8px", borderRadius: 4, border: "1px solid #CBD5E1", textAlign: "center" }}>
                  <div style={{ fontSize: 8, color: "#475569", fontWeight: 700, textTransform: "uppercase" }}>Opening Payable</div>
                  <div style={{ fontSize: 11, fontWeight: 800, color: "#1E293B", marginTop: 2 }}>{pkr(statementData.openingPayable)}</div>
                </div>
                <div style={{ background: "#FFFFFF", padding: "6px 8px", borderRadius: 4, border: "1px solid #CBD5E1", textAlign: "center" }}>
                  <div style={{ fontSize: 8, color: "#D97706", fontWeight: 700, textTransform: "uppercase" }}>Total Bills / Exp (+)</div>
                  <div style={{ fontSize: 11, fontWeight: 800, color: "#D97706", marginTop: 2 }}>{pkr(statementData.totalExpenses)}</div>
                </div>
                <div style={{ background: "#FFFFFF", padding: "6px 8px", borderRadius: 4, border: "1px solid #CBD5E1", textAlign: "center" }}>
                  <div style={{ fontSize: 8, color: "#059669", fontWeight: 700, textTransform: "uppercase" }}>Total Payments (-)</div>
                  <div style={{ fontSize: 11, fontWeight: 800, color: "#059669", marginTop: 2 }}>{pkr(statementData.totalPayments)}</div>
                </div>
                <div style={{ background: "#FEF3C7", padding: "6px 8px", borderRadius: 4, border: "1px solid #D97706", textAlign: "center" }}>
                  <div style={{ fontSize: 8, color: "#B45309", fontWeight: 800, textTransform: "uppercase" }}>Closing Payable</div>
                  <div style={{ fontSize: 11.5, fontWeight: 900, color: "#B45309", marginTop: 2 }}>{pkr(statementData.closingPayable)}</div>
                </div>
              </div>

              {/* TAX BREAKDOWN BANNER IN VENDOR PRINT MODAL */}
              {(statementData.totalGrossBilled > 0 || statementData.totalPayments > 0) && (
                <div style={{ display: "flex", justifyContent: "space-between", background: "#FFFBEB", border: "1px solid #FDE68A", borderRadius: 4, padding: "4px 8px", marginBottom: 8, fontSize: 8, color: "#78350F" }}>
              ...
              <table style={{ width: "100%", borderCollapse: "collapse", marginBottom: 10, fontSize: 8.5, tableLayout: "fixed" }}>
                ...
                <thead>
                  <tr style={{ background: "#F1F5F9", color: "#0F172A" }}>
                    ...
                    <th style={{ border: "1px solid #CBD5E1", padding: "5px 4px", textAlign: "left", fontSize: 8.2, fontWeight: 700 }}>PROJECT / EXPENSE</th>
                    <th style={{ border: "1px solid #CBD5E1", padding: "4px 3px", textAlign: "right", fontSize: 7.8, fontWeight: 700, lineHeight: 1.15 }}>DEBIT / PAYMENT<br/>(PKR)</th>
                    <th style={{ border: "1px solid #CBD5E1", padding: "4px 3px", textAlign: "right", fontSize: 7.8, fontWeight: 700, lineHeight: 1.15 }}>CREDIT / BILL<br/>(PKR)</th>
                    <th style={{ border: "1px solid #CBD5E1", padding: "4px 3px", textAlign: "right", fontSize: 7.8, fontWeight: 700, lineHeight: 1.15 }}>PAYABLE BALANCE<br/>(PKR)</th>
                  </tr>
                </thead>
                <tbody>
                  <tr style={{ background: "#F8FAFC", fontWeight: 600 }}>
                    ...
                    <td style={{ border: "1px solid #E2E8F0", padding: "4px 3px", textAlign: "right" }}>—</td>
                    <td style={{ border: "1px solid #E2E8F0", padding: "4px 3px", textAlign: "right", fontWeight: 700 }}>{pkr(statementData.openingPayable)}</td>
                  </tr>
                  ...
                  {statementData.rows.length === 0 ? (
                    ...
                    statementData.rows.map((row, idx) => (
                      <tr key={idx}>
                        ...
                        <td style={{ border: "1px solid #E2E8F0", padding: "4px 3px", textAlign: "center", fontWeight: 600, fontSize: 8.2, color: row.type === "Vendor Bill" ? "#D97706" : "#059669" }}>
                          {row.type}
                        </td>
                        <td style={{ border: "1px solid #E2E8F0", padding: "4px 4px", wordBreak: "break-word", fontSize: 8.5 }}>
                          ...
                          {row.taxBreakdown && row.taxBreakdown.kind === "bill" && (
                            <div style={{ fontSize: 7.2, color: "#92400E", marginTop: 2, lineHeight: 1.2 }}>
                        ...
                        <td style={{ border: "1px solid #E2E8F0", padding: "4px 3px", textAlign: "right", color: row.debit > 0 ? "#059669" : "inherit", fontSize: 8.5, fontWeight: row.debit > 0 ? 700 : 400 }}>
                          {row.debit > 0 ? pkr(row.debit) : "—"}
                        </td>
                        <td style={{ border: "1px solid #E2E8F0", padding: "4px 3px", textAlign: "right", color: row.credit > 0 ? "#D97706" : "inherit", fontSize: 8.5, fontWeight: row.credit > 0 ? 700 : 400 }}>
                          {row.credit > 0 ? pkr(row.credit) : "—"}
                        </td>
                        <td style={{ border: "1px solid #E2E8F0", padding: "4px 3px", textAlign: "right", fontWeight: 700, color: row.runningBalance > 0 ? "#D97706" : "#059669", fontSize: 8.8 }}>
                ...
                <tfoot>
                  <tr style={{ background: "#F1F5F9", fontWeight: 700 }}>
                    ...
                    <td style={{ border: "1px solid #CBD5E1", padding: "5px 3px", textAlign: "right", color: "#059669", fontSize: 8.5 }}>{pkr(statementData.totalPayments)}</td>
                    <td style={{ border: "1px solid #CBD5E1", padding: "5px 3px", textAlign: "right", color: "#D97706", fontSize: 8.5 }}>{pkr(statementData.totalExpenses)}</td>
                    <td style={{ border: "1px solid #CBD5E1", padding: "5px 3px", textAlign: "right", color: "#B45309", fontSize: 9 }}>{pkr(statementData.closingPayable)}</td>
                  </tr>
                </tfoot>
              </table>
